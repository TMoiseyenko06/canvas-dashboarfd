...
export default function Settings() {
  ...
  async function clearCache() {
    ...
    setClearing(false)
    setStatus(resp.ok
      ? { ok: true, msg: 'Cache cleared and re-sync triggered.' }
      : { ok: false, msg: 'Failed to clear cache.' })
  }
  ...
  return (
    <div className="max-w-lg space-y-8">
      ...
      <form onSubmit={saveSettings} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Wiggle Room Buffer (hours)</label>
        ...
        <div className="pt-4 border-t border-gray-100 space-y-4">
          <div className="flex items-center gap-2">
            <h3 className="text-sm font-semibold text-gray-700 uppercase tracking-wide">Telegram Notifications</h3>
            {telegramConfigured && (
              <span className="text-xs bg-green-100 text-green-700 rounded-full px-2 py-0.5 font-medium">Configured</span>
            )}
          </div>
          <p className="text-xs text-gray-500">
            Get notified on Telegram (works on iPhone, Android, desktop) when an assignment is due within 72 hours.
            <br />
            <a href="https://t.me/BotFather" target="_blank" rel="noreferrer" className="text-blue-500 hover:underline">
              1. Create a bot with @BotFather
            </a>
            {' · '}
            <a href="https://t.me/userinfobot" target="_blank" rel="noreferrer" className="text-blue-500 hover:underline">
          ...
          <div>
            ...
            <input type="password" value={form.telegram_bot_token}
              ...
              placeholder={telegramConfigured ? 'Leave blank to keep existing' : '123456:ABCdef...'}
          ...
          <div>
            ...
            <input type="text" value={form.telegram_chat_id}
              ...
              placeholder={telegramConfigured ? 'Leave blank to keep existing' : '123456789'}
              className={inputClass} />
          </div>
        </div>
      ...
      {hiddenCourses.length > 0 && (
        <div className="pt-2 border-t border-gray-200">
          <h3 className="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-3">Hidden Courses</h3>
          <div className="space-y-2">
            {hiddenCourses.map(c => (
              <div key={c.id} className="flex items-center justify-between bg-gray-50 rounded-lg px-3 py-2">
                <span className="text-sm text-gray-700">{c.name}</span>
                <button onClick={() => unhide(c.id)} className="text-xs text-blue-600 hover:underline">Unhide</button>
              </div>
            ))}
          </div>
        </div>
      ...
      {status && (
        <div className={`p-3 rounded-lg text-sm ${status.ok ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'}`}>
          {status.msg}
        </div>
      )}
    </div>
