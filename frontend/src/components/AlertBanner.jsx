...
export default function AlertBanner({ alerts }) {
  ...
  useEffect(() => {
    ...
    alerts
      .filter(a => !dismissed.has(a.id) && (a.urgency === 'overdue' || a.urgency === 'alert_active'))
      .forEach(a => {
        new Notification(`Canvas Alert: ${a.name}`, {
          body: `${a.urgency === 'overdue' ? 'OVERDUE' : 'Due soon'} — ${a.course_name}`,
          tag: String(a.id),
        })
      })
  }, [alerts, notifGranted])
  ...
  return (
    <div className="space-y-2 mb-4">
      {visible.map(a => (
        <div
          key={a.id}
          className={`flex items-start justify-between rounded-lg px-4 py-3 shadow ${URGENCY_STYLES[a.urgency] || 'bg-gray-200'}`}
        >
          <div>
