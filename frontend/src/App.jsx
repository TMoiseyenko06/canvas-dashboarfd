...
const TABS = ['Courses', 'Assignments', 'Calendar', 'GPA', 'Settings']
...
function useSyncPoller() {
  ...
  const poll = useCallback(async () => {
    try {
      ...
      if (s.syncing) timerRef.current = setTimeout(poll, 1500)
    } catch {
...
export default function App() {
  ...
  useEffect(() => {
    if (prevSyncing.current && !syncing) {
      reloadCourses(); reloadAssignments(); reloadAlerts()
    }
  ...
  async function handleOverride(assignmentId, hours) {
    ...
    await reloadAssignments(); await reloadAlerts()
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        {syncing && (
          <div className="flex items-center gap-3 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-700 rounded-xl px-4 py-3 mb-4 text-blue-800 dark:text-blue-300 text-sm">
        ...
        {!syncing && syncError && (
          <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-700 rounded-xl px-4 py-3 mb-4 text-red-800 dark:text-red-300 text-sm">
        ...
        {!isLoading && hasError && (
          <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-700 rounded-xl p-5 text-red-700 dark:text-red-300">
            ...
            <p className="text-sm mt-2 opacity-75">Make sure your Canvas API token and base URL are configured in Settings.</p>
          </div>
        )}
        ...
        {!isLoading && !hasError && (
          <>
            {tab === 'Courses' && (
              <div>
                <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100 mb-4">
                  Your Courses <span className="text-sm font-normal text-gray-400">({courses?.length ?? 0})</span>
                </h2>
                {!syncing && courses?.length === 0 && (
                  <div className="text-center py-10 text-gray-400">
                    <p>No active courses found.</p>
                    <p className="text-sm mt-1">Check your Canvas token and base URL in Settings.</p>
                  </div>
                )}
            ...
            {tab === 'Assignments' && (
              <div>
                <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100 mb-4">
                  Assignments <span className="text-sm font-normal text-gray-400">({assignments?.length ?? 0})</span>
                </h2>
                <AssignmentList assignments={assignments ?? []} courses={courses ?? []} onOverride={handleOverride} />
              </div>
            )}

            {tab === 'Calendar' && (
              <div>
                <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100 mb-4">Calendar</h2>
            ...
            {tab === 'GPA' && (
              <div>
                <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100 mb-4">GPA Calculator</h2>
